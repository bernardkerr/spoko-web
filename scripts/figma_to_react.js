...
const FIGMA_ACCESS_TOKEN = process.env.FIGMA_ACCESS_TOKEN || process.env.FIGMA_PERSONAL_ACCESS_TOKEN;
const FIGMA_FILE_KEY = process.env.FIGMA_FILE_KEY;
const FIGMA_BRANCH = process.env.FIGMA_BRANCH;

if (!FIGMA_ACCESS_TOKEN || !FIGMA_FILE_KEY) {
  console.error('❌ Missing Figma configuration. Please set FIGMA_ACCESS_TOKEN (or FIGMA_PERSONAL_ACCESS_TOKEN) and FIGMA_FILE_KEY in .env.local');
  process.exit(1);
}
...
class FigmaToReactGenerator {
  ...
  async generateDesignPage(fileKey) {
    ...
    try {
      ...
      const fileData = await this.figmaAPI.getFile(fileKey, FIGMA_BRANCH ? { branch: FIGMA_BRANCH } : undefined);
      
      console.log(`📄 Found file: ${fileData.name}`);
      console.log(`📑 Pages: ${fileData.document.children.length}`);
      if (FIGMA_BRANCH) console.log(`🌿 Branch: ${FIGMA_BRANCH}`);
