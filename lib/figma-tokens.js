...
export default {
  theme: {
    extend: ${JSON.stringify(tailwindConfig, null, 2)}
      ...
      // Also copy to public folder for web access in static export mode
      const publicPath = './public/tailwind.figma.js';
      await fs.writeFile(publicPath, configContent);
      console.log(`Tailwind config copied to ${publicPath} for web access`);
...
`;
  ...
  async generateComponents(fileKey, componentIds, outputDir = './components/figma') {
    try {
      ...
      // Create a manifest file for the UI to load
      const manifest = {
        components: components.map(comp => ({
          id: comp.name.toLowerCase(),
          name: comp.name,
          description: `Generated from Figma component`,
          category: 'Generated',
          properties: comp.props || {},
          filePath: `./components/figma/${comp.name.toLowerCase()}.jsx`
        })),
        lastSync: new Date().toISOString()
      };
      
      // Write manifest to public folder for web access
      await fs.writeFile('./public/figma-components.json', JSON.stringify(manifest, null, 2));
      console.log('Component manifest created at ./public/figma-components.json');
